...
app.get("/api/url", async (req, res) => {
  ...
  if (finalParsed.protocol === "https:") {
    try {
      result.ssl = await withTimeout(
        new Promise((resolve, reject) => {
          const socket = tls.connect(
            { host: finalParsed.hostname, port: 443, servername: finalParsed.hostname, timeout: 6000 },
            () => {
  ...
  // Reputation: urlscan.io public search (no API key needed)
  try {
    const q = encodeURIComponent(`domain:${finalParsed.hostname}`);
    const r = await withTimeout(fetch(`https://urlscan.io/api/v1/search/?q=${q}&size=5`), 6000);
    if (r.ok) {
    ...
    result.reputation = { error: "reputation lookup unavailable" };
  }
